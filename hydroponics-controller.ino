...
static void esp_zb_temp_sensor_handler(float temperature) {
  ...
  switch (sig_type) {
    ...
    case ESP_ZB_BDB_SIGNAL_DEVICE_REBOOT:
      if (err_status == ESP_OK) {
        ...
        // Start Water level control task
        xTaskCreate(water_level_update, "water_level_update", 2048, NULL, 10, NULL);
...
void relaySetup() {
    log_i("Relay configured to pin %d", RELAY_GPIO);
...
void relayToggle() {
    ...
    log_i("Toggling relay to state: %d", relayState);
    gpio_set_level(RELAY_GPIO, relayState);
}

void relaySet(uint8_t level) {
    log_i("Setting relay to state: %d", level);
...
static water_state_t currentState = EMPTYING;

void setupWaterSwitches() {
    log_i("Switches configured on pins high [%d] and low [%d]", WATER_LEVEL_HIGH_GPIO, WATER_LEVEL_LOW_GPIO);
...
uint8_t shouldTankBeRefilled() {
    ...
    log_i("highState [%d], lowState [%d]", highState, lowState);

    if (highState && !lowState) {
        log_e("Something is wrong with the configuration, state should not be possible (high = 1, low = 0)");
        return 0;
    } else if (!highState && lowState) {
        log_i("Detected level below max, currentState is [%d] (0 = FILLING, 1 = EMPTYING), will return [%d]", 
        currentState, currentState == FILLING);
        return currentState == FILLING;
    } else if (highState && lowState) {
        log_i("Detected level at max, currentState is [%d] (0 = FILLING, 1 = EMPTYING), will return [0]", 
        currentState);
        currentState = EMPTYING;
        return 0;
    } else {
        log_i("Detected level below low, currentState is [%d] (0 = FILLING, 1 = EMPTYING), will return [1]", 
        currentState);
...
static void water_level_update(void *arg) {
  for (;;) {
...
static void temp_sensor_value_update(void *arg) {
  for (;;) {
    float tsens_value = 12.0;
    esp_zb_temp_sensor_handler(tsens_value);
    delay(1000);
  }
}
